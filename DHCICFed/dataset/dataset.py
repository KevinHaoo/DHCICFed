...
class SkinDataset(Dataset):
    def __init__(self, root, mode, transform=None):
        ...
        if self.mode == "train":
            ...
            self.images = self.images[:int(0.4 * initial_len)]
            self.targets = self.targets[:int(0.4 * initial_len)]
        elif self.mode == "valid":
            # self.images = self.images[int(0.7*initial_len):int(0.8*initial_len)]
            # self.targets = self.targets[int(0.7*initial_len):int(0.8*initial_len)]
            self.images = self.images[int(0.3 * initial_len):int(0.4 * initial_len)]
            self.targets = self.targets[int(0.3 * initial_len):int(0.4 * initial_len)]
        else:
    ...
    def __getitem__(self, index):
        ...
        """
        image_name = os.path.join(self.root, "train", self.images[index] + ".jpg")
...
class ichDataset(Dataset):
    def __init__(self, root, mode, transform=None):
        ...
        if self.mode == "train":
            ...
            # self.targets = self.targets[:int(0.7*initial_len)]
            self.image_id = self.image_id[:int(0.4*initial_len)]
            self.targets = self.targets[:int(0.4*initial_len)]
        elif self.mode == "valid":
            # self.image_id = self.image_id[int(0.7*initial_len):int(0.8*initial_len)]
            # self.targets = self.targets[int(0.7*initial_len):int(0.8*initial_len)]
            self.image_id = self.image_id[int(0.3 * initial_len):int(0.4 * initial_len)]
            self.targets = self.targets[int(0.4 * initial_len):int(0.4 * initial_len)]
        else:
